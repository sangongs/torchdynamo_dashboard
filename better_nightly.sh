...
scratch_dir="${HOME}/"
top_dir="${HOME}/dashboard"
work_dir="${HOME}/dashboard/work"
env_dir="${HOME}/dashboard/env"
zipped_file="~/dashboard.tar.lz4"
#rm -rf $top_dir
mkdir -p $top_dir
mkdir -p $work_dir
...
#lz4 -dc < $zipped_file | tar xf - -C $scratch_dir

echo "#### bash: Activating conda environment from $env_dir ####"
#alias with-proxy='HTTPS_PROXY=http://fwdproxy:8080 HTTP_PROXY=http://fwdproxy:8080 FTP_PROXY=http://fwdproxy:8080 https_proxy=http://fwdproxy:8080 http_proxy=http://fwdproxy:8080 ftp_proxy=http://fwdproxy:8080'
...
git config --global https.proxy http://fwdproxy:8080
...
make -f Makefile_dashboard clone-deps
